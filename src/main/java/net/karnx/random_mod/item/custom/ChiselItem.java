...
public class ChiselItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        if(Screen.hasShiftDown()) {
            tooltip.add(Text.translatable("tooltip.random-mod.chisel.shift_down"));
            tooltip.add(Text.translatable("tooltip.random-mod.chisel.shift_down.2"));
        } else {
            tooltip.add(Text.translatable("tooltip.random-mod.chisel"));
        }

        if (stack.get(ModDataComponentTypes.COORDINATES) != null) {
            tooltip.add(Text.literal("Last Block Chisel Used at " + stack.get(ModDataComponentTypes.COORDINATES)));
        }
